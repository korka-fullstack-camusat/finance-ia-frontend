...
export function ArtifactPanel() {
  ...
  // Résultat affiché : le dernier run manuel, ou celui sélectionné dans la liste
  const activeResult =
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="h-12 bg-white border-b border-[#E2E8F0] flex items-center justify-between px-5 shrink-0">
        ...
        {activeResult && (
          <span className="text-xs text-[#64748B]">
            {formatDate(activeResult.created_at)}
          </span>
        )}
      </div>

      {/* Progress + Logs (only when running) */}
      {isRunning && (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-40 shrink-0 border-r border-[#E2E8F0] overflow-y-auto bg-[#F8FAFC]">
          {(results || []).map((r) => {
            ...
            return (
              ...
              >
                <p className="text-lg leading-none mb-1">{TASK_ICONS[task?.task_type ?? ""] || "📋"}</p>
        ...
        <div className="flex-1 overflow-y-auto bg-white">
          {activeResult ? (
            ...
            /* Vide — pas de message, juste blanc */
  ...
  return (
    <div className="flex flex-col h-full animate-slideIn">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-[#F1F5F9] bg-[#F8FAFC]">
        <div>
          <p className="text-xs font-semibold text-[#0F172A]">{taskName}</p>
